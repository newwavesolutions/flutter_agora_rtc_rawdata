#include "devileffect.h"
#include "../common/blend.h"

cv::Mat DevilEffect::apply(cv::Mat &to, const void *data) {
...
#ifdef IMG_DEBUG_LOG
  std::cout << "devil effect : " << t*1000 << " milliseconds" << std::endl;
#endif
  return to;
}

DevilEffect::~DevilEffect() {
  std::cout << "destroy devil effect" << std::endl;
}
