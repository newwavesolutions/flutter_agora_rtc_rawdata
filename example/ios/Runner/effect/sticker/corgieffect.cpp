#include "corgieffect.h"
#include "../common/blend.h"

cv::Mat CorgiEffect::apply(cv::Mat &to, const void *data) {
  ...
  // move up
  int distance = (left.y - nose.y) * 0.15;
  left.y -= distance;
  right.y -= distance;
...
#ifdef IMG_DEBUG_LOG
  std::cout << "corgi effect : " << t*1000 << " milliseconds" << std::endl;
#endif
  return to;
}

CorgiEffect::~CorgiEffect() {
  std::cout << "destroy corgi effect" << std::endl;
}
