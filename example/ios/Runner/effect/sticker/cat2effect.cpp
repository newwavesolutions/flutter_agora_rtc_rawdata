#include "cat2effect.h"
#include "../common/blend.h"

cv::Mat Cat2Effect::apply(cv::Mat &to, const void *data) {
...
#ifdef IMG_DEBUG_LOG
  std::cout << "cat2 effect : " << t*1000 << " milliseconds" << std::endl;
#endif
  return to;
}

Cat2Effect::~Cat2Effect() {
  std::cout << "destroy cat effect" << std::endl;
}
