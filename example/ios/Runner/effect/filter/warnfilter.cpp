#include "./warnfilter.h"

void WarnFilter::init(const effect_config &config) {
...
cv::Mat WarnFilter::apply(cv::Mat &to, const void *data) {
  ...
  cv::split(to, bgr);
  cv::Mat r, b;
  cv::LUT(bgr[ir], this->incr_ch_lut, r);
  cv::LUT(bgr[ib], this->decr_ch_lut, b);
  ...
  cv::cvtColor(to, img_hsv, cv::COLOR_BGR2HSV);
  cv::split(to, hsv);
  cv::Mat s;
  cv::LUT(hsv[1], incr_ch_lut, s);
...
#ifdef IMG_DEBUG_LOG
  std::cout << "duration applying warm filter : " << t*1000 << " milliseconds" << std::endl;
#endif
  return to;
}
